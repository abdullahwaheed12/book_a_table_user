      ...
      if (query.docs.isNotEmpty) {
        FirebaseFirestore.instance.collection('orders').add({
          ...
          Get.find<GeneralController>()
              ...
              ?.update({'orderReference': value.id}).then((value) {
            Get.find<GeneralController>().tableBookingDocumentReference = null;
          });
        });
        ...
        //!-----------------------
        try {
          sendNotificationCall(queryForFcm.docs[0].get('token'),
              'Order received ${query.docs[0].get('name')} ', ' : ');
          print('notification for restuarent');
        } on FirebaseException catch (e) {
          print(
              'for personal notifcation sending exception with messsage ${e.message}');
        } catch (e) {
